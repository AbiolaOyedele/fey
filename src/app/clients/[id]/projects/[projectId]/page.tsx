...
export default function ProjectDetailPage({ params }: { params: Promise<{ id: string; projectId: string }> }) {
  ...
  // Adapt project rows to the shapes the shared CRM components expect.
  const threadMessages = useMemo<CrmMessage[]>(
    () => messages.map((m) => ({
      id: m.id,
      contact_id: m.project_id,
      owner_id: m.owner_id,
  ...
  const handleSend = useCallback(
    async (text: string, html: string, attachments: MessageAttachment[]) => {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-shrink-0 bg-white border-b border-gray-100 px-4 lg:px-6 py-3">
        <div className="flex items-center gap-3">
          <button
            onClick={() => router.push(`/clients/${id}/projects`)}
