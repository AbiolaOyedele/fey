...
function NewInvoicePageInner() {
  ...
  const [invoiceNum, setInvoiceNum] = useState<string>(initNum())
  const [status,     _setStatus]    = useState<string>('draft')
  ...
  const [taskIds,   _setTaskIds]  = useState<string[]>([])
  ...
  const [notes,          setNotes]          = useState<string>(settings.default_invoice_notes || '')
  ...
  // ── Pre-fill from query params (replaces router state) ─────────────────────

  useEffect(() => {
    const prefillClientId = searchParams?.get('prefillClientId')
    if (!prefillClientId) return
    // Fetch the client from supabase if a prefillClientId was provided
    void (async () => {
      const { data } = await supabase
        .from('clients')
        .select('id, name, email, phone, address, website, tax_id')
      ...
      if (data) {
        setBillTo({
          ...
          tax_id:    (data as { tax_id?: string }).tax_id   || '',
        })
      }
    })()
  }, [searchParams])
  ...
  const doSave = async (saveStatus: string): Promise<{ data?: { id: string }; error?: string }> => {
    ...
    if (savedId) {
      result = await updateInvoice(savedId, payload as Partial<Invoice>)
    } else {
      result = await createInvoice(payload as Partial<Invoice>)
      if (result.data) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-100">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3 sticky top-0 z-30 no-print">
        <button onClick={() => router.push('/invoices')} className="p-2 rounded-xl hover:bg-gray-100 text-gray-500 transition-colors">
          <ArrowLeft size={18} />
        </button>
