...
function DraggableTaskItem({ task, onUpdate, onDelete }: DraggableTaskItemProps) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: task.id })
  const style = { transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.4 : 1 }
  return (
    <div ref={setNodeRef} style={style}>
      <SimpleTaskItem task={task} onUpdate={onUpdate} onDelete={onDelete} dragListeners={listeners} dragAttributes={attributes as unknown as Record<string, unknown>} />
    </div>
  )
}
...
export default function TaskGroupWorkspacePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  ...
  const {
    groups, updateGroup, addGroupTask, updateGroupTask, removeGroupTask, reorderGroupTasks,
  ...
  const handleAddTask = async () => {
    ...
    await addGroupTask(id, newTask.trim())
    setNewTask('')
  }

  const handleUpdateTask = async (updatedTask: StandaloneTask) => {
    await updateGroupTask(id, updatedTask.id, {
      title: updatedTask.title, done: updatedTask.done, deadline: updatedTask.deadline,
    })
  }
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    void reorderGroupTasks(id, arrayMove(group.tasks, oldIndex, newIndex).map((t) => t.id))
  }, [group.tasks, id, reorderGroupTasks])

  const filterTaskList = (tasks: StandaloneTask[]): StandaloneTask[] => {
  ...
  const handleSaveEdit = async () => {
    ...
    await updateGroup(id, { name: editName.trim(), color: editColor, icon: editIcon })
    setEditOpen(false)
  }
      ...
      <div className="flex-1 p-4 lg:p-8 lg:pr-4 min-w-0 overflow-y-auto overflow-x-hidden">
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden mb-4">
          <DndContext
            ...
            {...(dndEnabled ? { onDragEnd: handleDragEnd } : {})}
          >
                  ...
                  {pendingTasks.map((task) =>
                    dndEnabled ? (
                      <DraggableTaskItem
                        ...
                        onUpdate={(t) => void handleUpdateTask(t)}
                        onDelete={(tid) => void handleDeleteTask(tid)}
                      />
                    ) : (
                      <SimpleTaskItem
                        key={task.id} task={task}
          ...
          <div className="px-4 py-3 border-t border-gray-100 flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-5 py-2 text-white rounded-full text-sm font-medium hover:opacity-90 disabled:opacity-30 transition-all flex-shrink-0"
              ...
              <div className="flex items-center gap-2">
                {editCustomHex && isValidHex(editCustomHex) && (
                  <div
                    ...
                    style={{ backgroundColor: normalizeHex(editCustomHex) }}
                  />
                )}
                <input
                  type="text"
