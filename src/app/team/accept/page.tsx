...
function AcceptInviteInner() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!pending) { if (active) { setState('error'); setMessage('This invite link is missing its token.') } return }
      ...
      try {
        ...
        if (!active) return
        setState('done')
        setTimeout(() => router.replace('/playground'), 1400)
      } catch (e) {
        if (active) { setState('error'); setMessage(e instanceof Error ? e.message : 'Something went wrong.') }
      }
    })()
    return () => { active = false }
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 max-w-sm w-full text-center">
        {state === 'working' && (
          <>
            ...
            <p className="text-sm font-medium text-gray-700">Accepting your invite…</p>
          </>
        )}
        {state === 'need-auth' && (
          <>
        ...
        {state === 'done' && (
          <>
            ...
            <p className="text-sm font-medium text-gray-800">You’re in! Taking you to your team…</p>
          </>
        )}
        {state === 'error' && (
          <>
