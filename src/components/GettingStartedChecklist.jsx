...
export default function GettingStartedChecklist({ clients = [] }) {
  ...
  let stored = {};
  try { stored = JSON.parse(settings.checklist_steps || '{}'); } catch { /* ignore */ }
          ...
          {STEPS.map((step) => {
            ...
            return (
              ...
              >
                ...
                {!done && isManual && (
                  <span className="ml-auto text-[10px] text-gray-400 italic flex-shrink-0">click to mark</span>
                )}
              </button>
            );
          })}
