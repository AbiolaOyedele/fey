...
interface TaskAttachmentsProps {
  taskId: string
  files: TaskFileRow[]
  onAdd: (taskId: string, payload: { file_name: string; file_url: string; public_id: string; file_size?: number | null; file_type?: string | null }) => Promise<unknown>
  onRemove: (taskId: string, fileId: string) => Promise<void>
  /** Hide upload/delete affordances (portal read-only rendering). */
  readOnly?: boolean
}

/**
 * Attachments section for the task drawer: drag-drop/browse upload (direct to
 * Cloudinary via the signed flow), thumbnail grid, image lightbox with
 * download, per-file delete. Mirrors the legacy TaskFileAttachment UX on the
 * new work-tasks data model.
 */
export default function TaskAttachments({ taskId, files, onAdd, onRemove, readOnly = false }: TaskAttachmentsProps) {
  const [uploads, setUploads] = useState<UploadItem[]>([])
  const [dragOver, setDragOver] = useState(false)
  const [failed, setFailed] = useState<string | null>(null)
  const [preview, setPreview] = useState<TaskFileRow | null>(null)
  ...
  const openFile = useCallback((f: TaskFileRow) => {
    ...
    <div>
      {!readOnly && (
        ...
        >
          ...
          <span className="text-xs2 text-gray-500">
            Drop files or{' '}
      ...
      {failed && (
        <p className="mt-2 text-xs2 text-red-500">{failed}</p>
      )}
