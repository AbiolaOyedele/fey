...
type Step = 'choose' | 'tasks' | 'client'
type TaskFilterValue = 'all' | 'unpaid' | 'paid'
...
export default function NewInvoiceModal({ clients = [], onClose }: NewInvoiceModalProps) {
  const router = useRouter()
  const [step, setStep] = useState<Step>('choose')
  const [taskFilter, setTaskFilter] = useState<TaskFilterValue>('all')
  const [selectedTasks, setSelectedTasks] = useState<Record<string, SelectedTask>>({})
  const [selectedClientId, setSelectedClientId] = useState<string>('')
  const [clientTaskFilter, setClientTaskFilter] = useState<TaskFilterValue>('all')

  // Build full task list grouped by client
  const allTaskGroups: TaskGroup[] = clients
  ...
  const toggleTask = (task: Task, client: Client) => {
    setSelectedTasks((prev) => {
      const next = { ...prev }
      if (next[task.id]) {
        delete next[task.id]
      } else {
        next[task.id] = { task, client }
      }
      return next
    })
  }

  const toggleAllForGroup = (group: TaskGroup) => {
    ...
    setSelectedTasks((prev) => {
      ...
      if (allSelected) {
        ...
        group.tasks.forEach((t) => {
          next[t.id] = { task: t, client: group.client }
        })
      }
      return next
    })
  ...
  const confirmTaskSelection = () => {
    ...
    entries.forEach(({ client }) => {
      clientCounts[client.id] = (clientCounts[client.id] ?? 0) + 1
    })
    ...
    const lineItems: LineItem[] = entries.map(({ task }) => ({
      id: `task-${task.id}`,
      description: task.title,
      qty: 1,
      price: parseFloat(String(task.amount)) || 0,
      amount: parseFloat(String(task.amount)) || 0,
      task_id: task.id,
    }))

    router.push(
      `/invoices/new?state=${encodeURIComponent(
        JSON.stringify({
  ...
  const confirmClientSelection = () => {
    ...
    const lineItems: LineItem[] = tasks.map((t) => ({
      id: `task-${t.id}`,
      description: t.title,
      qty: 1,
      price: t.amount || 0,
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
          {step !== 'choose' && (
            <button
              ...
              className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-800 transition-colors mr-3"
            >
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-xl hover:bg-gray-100 text-gray-400 ml-auto"
          >
            <X size={18} />
          </button>
        </div>
        ...
        {step === 'tasks' && (
          <>
                ...
                {(['all', 'unpaid', 'paid'] as const).map((f) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-medium capitalize transition-colors ${
                      taskFilter === f ? 'text-white' : 'bg-gray-100 text-gray-500'
                    }`}
                    style={taskFilter === f ? { backgroundColor: 'var(--accent)' } : {}}
                  >
                    {f === 'all' ? 'All Tasks' : f === 'unpaid' ? 'Unpaid' : 'Paid'}
                  </button>
                ))}
              </div>
            </div>
                    ...
                    return (
                      <div key={client.id}>
                        <div className="flex items-center gap-2 mb-2">
                          <button
                            onClick={() => toggleAllForGroup({ client, tasks })}
                            className="flex items-center gap-2"
                          >
                            <div
                              className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-colors ${
                                allSel ? 'border-transparent' : 'border-gray-300'
                              }`}
                              style={allSel ? { backgroundColor: 'var(--accent)', borderColor: 'var(--accent)' } : {}}
                            >
                        ...
                        <div className="space-y-1 pl-6">
                          {tasks.map((task) => {
                            ...
                            return (
                              <button
                                key={task.id}
                                onClick={() => toggleTask(task, client)}
                              ...
                              >
                                <div
                                  className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                                    sel ? 'border-transparent' : 'border-gray-300'
                                  }`}
                                  style={sel ? { backgroundColor: 'var(--accent)', borderColor: 'var(--accent)' } : {}}
                                >
                                ...
                                {task.amount > 0 && (
                                  <span className="text-xs font-medium text-gray-500 flex-shrink-0">
                                    {task.currency || 'NGN'} {task.amount.toLocaleString()}
                                  </span>
                                )}
                                {task.paid && (
            ...
            <div className="px-6 pb-5 pt-3 border-t border-gray-100 flex-shrink-0">
              ...
              >
                {selectedCount > 0
                  ? `Create Invoice with ${selectedCount} task${selectedCount !== 1 ? 's' : ''}`
                  ...
                  {clients.map((c) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border-2 text-left transition-all ${
                        selectedClientId === c.id ? '' : 'border-gray-100 hover:border-gray-200'
                      }`}
                      style={
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-400">
                          {c.tasks?.length ?? 0} task{c.tasks?.length !== 1 ? 's' : ''}
                        </p>
                      </div>
                      {selectedClientId === c.id && (
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {(['all', 'unpaid', 'paid'] as const).map((f) => (
                      <button
                        key={f}
                        onClick={() => setClientTaskFilter(f)}
                        className={`px-3 py-1.5 rounded-full text-xs font-medium capitalize transition-colors ${
                          clientTaskFilter === f ? 'text-white' : 'bg-gray-100 text-gray-500'
                        }`}
                  ...
                  {(() => {
                    ...
                    return (
                      <p className="text-xs text-gray-400 mt-2">
                        {count} task{count !== 1 ? 's' : ''} will be added as line items
                      </p>
                    )
                  })()}
                </div>
