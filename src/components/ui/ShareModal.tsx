...
export default function ShareModal({ client, userId, onClose }: ShareModalProps) {
  const { settings } = useSettings()
  const [shareRecord, setShareRecord] = useState<SharedClientRecord | null>(null)
  ...
  useEffect(() => {
    async function fetchAll() {
      ...
      const { data: share } = await supabase
        ...
        .eq('active', true)
        .maybeSingle()
      setShareRecord((share as SharedClientRecord | null) ?? null)
      ...
      if (share) {
        ...
        setInvites((codes as InviteRecord[]) ?? [])
      }
      setLoading(false)
    }
    void fetchAll()
  ...
  const handleCreate = async () => {
    ...
    setShareRecord(data as SharedClientRecord)
    setInvites([])
    setCreating(false)
  }
  ...
  const handleGenerateCode = async () => {
    ...
    const { data, error: err } = await supabase
      ...
      .single()
    if (!err && data) {
      setInvites((prev) => [data as InviteRecord, ...prev])
      setLabelInput('')
  ...
  const handleRevokeCode = async (invite: InviteRecord) => {
    await supabase
      .from('shared_client_invites')
      .update({ status: 'revoked' })
      .eq('id', invite.id)
    setInvites((prev) =>
      prev.map((inv) => (inv.id === invite.id ? { ...inv, status: 'revoked' as const } : inv))
    )
    if (invite.member_id) {
  ...
  const handleCopyCode = async (invite: InviteRecord) => {
    await navigator.clipboard.writeText(invite.code)
    setCopiedCode(invite.id)
    setTimeout(() => setCopiedCode(null), 2000)
  }

  const smartLink = (invite: InviteRecord) => `${shareLink}?code=${invite.code}`
  ...
  const statusBadge = (invite: InviteRecord): StatusBadge => {
    if (invite.status === 'revoked')
      return { label: 'Revoked', bg: '#FEF2F2', color: '#EF4444' }
    if (invite.status === 'used')
      return {
        label: `Used${invite.member_name ? ` · ${invite.member_name}` : ''}`,
        bg: '#D1FAE5',
        color: '#065F46',
      }
  ...
  // handleCopyCode is defined but only used optionally — keep for completeness
  void handleCopyCode
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">
                Generate a unique link per person — code included
              </p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-600 transition-colors flex-shrink-0"
          >
            <X size={16} />
          </button>
        </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-4 space-y-4">
              <div
                ...
                style={{ backgroundColor: 'var(--accent, #ED64A6)15' }}
              >
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Then generate unique invite codes for each person.
                </p>
              </div>
                ...
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">
                      Each generates a smart link — code is pre-filled for the receiver.
                    </p>
                  </div>
                  <button
                    ...
                    <div className="flex items-center gap-2 flex-1 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2">
                      ...
                      <input
                        ...
                        onKeyDown={(e: React.KeyboardEvent<HTMLInputElement>) => {
                          if (e.key === 'Enter') void handleGenerateCode()
                        }}
                ...
                {invites.length === 0 ? (
                  <div className="text-center py-6 rounded-xl border border-dashed border-gray-200">
                    ...
                    <p className="text-xs text-gray-300 mt-0.5">
                      Generate one for each person you want to invite.
                    </p>
                  </div>
                ) : (
