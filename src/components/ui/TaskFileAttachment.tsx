...
import { uploadToCloudinary, getFileType, formatFileSize, isImageType, type FileType } from '@/utils/cloudinary'
...
function FileTypeIcon({ fileType, size = 14 }: FileTypeIconProps) {
...
export default function TaskFileAttachment({ taskId, clientId, open }: TaskFileAttachmentProps) {
  const { user }                                     = useAuth()
  ...
  const handleFiles = useCallback(async (fileList: FileList | null) => {
    ...
    for (const file of accepted) {
      ...
      try {
        ...
        if (error) showToast(`Failed to save "${file.name}"`)
      } catch (err) {
        if (err instanceof Error && err.message !== 'cancelled') showToast(`Upload failed: ${err.message}`)
      } finally {
        delete abortsRef.current[uid]
        setUploads((p) => p.filter((u) => u.id !== uid))
      }
    }
  }, [taskId, clientId, user, settings, addFile, showToast])
        ...
        <div className="border-t border-gray-100">
          {files.map((f) => (
            <div key={f.id} className="group flex items-center gap-2.5 px-4 py-2 hover:bg-gray-100 transition-colors">
              {isImageType(f.file_type as FileType) ? (
                <img
                  src={f.file_url}
                  alt={f.file_name}
                ...
                <div className="w-7 h-7 rounded-lg bg-white flex items-center justify-center flex-shrink-0 text-gray-400 border border-gray-100">
                  <FileTypeIcon fileType={f.file_type as FileType} />
                </div>
              )}
              <a
                href={f.file_url}
