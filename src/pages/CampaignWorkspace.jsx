...
import {
  ...
  Upload, File, FileText, Image, Folder, X, Loader2, Trash2,
} from 'lucide-react';
import {
...
export default function CampaignWorkspace({ clients }) {
  ...
  // Use a ref for allTasks so handleDragEnd (defined here, before guards) can access them
  const allTasksRef       = useRef([]);

  const sensors = useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 8 } }));

  // handleDragEnd must be a useCallback at top level — cannot be after conditional returns
  const handleDragEnd = useCallback((event) => {
    ...
    reorderTasks(campaignId, arrayMove(tasks, oldI, newI).map((t) => t.id));
  ...
  if (!client) return (
    <div className="p-8 text-center py-20">
      ...
      <button onClick={() => navigate('/clients')} className="text-sm mt-2 hover:underline text-gray-500">Back to Clients</button>
    </div>
  );
  ...
  if (!campaign) return (
    <div className="p-8 text-center py-20">
      ...
      <button onClick={() => navigate(`/clients/${clientId}`)} className="text-sm hover:underline" style={{ color: client.color }}>
        Back to {client.name}
      </button>
    </div>
  );
  ...
  const uploadSingleFile = async (file) => {
    ...
    try {
      ...
      if (error) {
        showToast(error.message || 'Failed to save file record', 'error');
        return false;
      }
      return true;
    } catch (err) {
      showToast(err.message || `Upload failed: ${file.name}`, 'error');
      return false;
    } finally {
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen page-enter overflow-hidden max-w-full">
      ...
      <div className="flex-1 p-4 lg:p-8 lg:pr-4 min-w-0 overflow-y-auto overflow-x-hidden">
        ...
        <div className="rounded-2xl p-5 mb-5 overflow-hidden" style={{ backgroundColor: campaignColor }}>
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
            {campaign.logo ? (
              <img src={campaign.logo} alt={campaign.name} className="w-14 h-14 rounded-2xl object-contain bg-white/70 p-1 flex-shrink-0" />
            ) : (
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center bg-white/50 flex-shrink-0" style={{ color: textColor }}>
                <Layers size={26} />
              </div>
            )}
            <div className="flex-1 min-w-0">
              <h1 className="font-display text-2xl sm:text-3xl leading-tight font-bold truncate" style={{ color: textColor }}>
                {campaign.name}
              </h1>
        ...
        {activeTab === 'tasks' && (
          <div className="bg-white rounded-2xl shadow-sm p-4 sm:p-5">
            ...
            {filterDropdownOpen && (
              <div ref={filterDropdownRef} className="fixed bg-white rounded-xl shadow-lg border border-gray-100 z-50 py-1 w-40" style={{ top: filterPos.top, left: filterPos.left }}>
                {TASK_FILTER_OPTIONS.map((opt) => (
                  <button
                    key={opt.value}
                    onClick={() => { setTaskFilter(opt.value); setFilterDropdownOpen(false); }}
                    className={`w-full text-left px-3 py-2 text-xs font-medium transition-colors ${taskFilter === opt.value ? '' : 'text-gray-500 hover:bg-gray-50'}`}
              ...
              <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={() => { isDraggingRef.current = true; }} onDragEnd={handleDragEnd}>
                <SortableContext items={pendingTasks.map((t) => t.id)} strategy={verticalListSortingStrategy}>
                  {pendingTasks.map((task) => (
                    <SortableTaskRow key={task.id} task={task} onUpdate={handleUpdateTask} onDelete={() => deleteTask(campaignId, task.id)} />
                  ))}
                </SortableContext>
              </DndContext>
            ) : (
        ...
        {activeTab === 'files' && (
          <div className="bg-white rounded-2xl shadow-sm p-4 sm:p-5">
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm font-semibold text-gray-700">
                Files {files.length > 0 && <span className="font-normal text-gray-400">{files.length} total</span>}
              </p>
              <button
            ...
            {uploads.map((u) => (
              <div key={u.id} className="bg-gray-50 rounded-xl p-3 mb-3">
                ...
                <div className="h-1 bg-gray-200 rounded-full overflow-hidden">
                  <div className="h-full rounded-full transition-all" style={{ width: `${u.progress}%`, backgroundColor: campaignColor }} />
                </div>
              </div>
            ))}
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm mb-4">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: `${campaignColor}22` }}>
            ...
            {files.length > 0 && (
              <button onClick={() => setActiveTab('files')} className="flex items-center gap-3 w-full text-left hover:bg-gray-50 rounded-xl -mx-2 px-2 py-1 transition-colors">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: `${campaignColor}22` }}>
          ...
          {allTasks.length > 0 && (
            <div className="mt-4 pt-4 border-t border-gray-100">
              ...
              <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                <div className="h-full rounded-full transition-all duration-500" style={{ width: `${(allTasks.filter((t) => t.done).length / allTasks.length) * 100}%`, backgroundColor: campaignColor }} />
              </div>
            </div>
          )}
        </div>
      ...
      {editModalOpen && (
        <EditCampaignModal
          campaign={campaign}
          onSave={async (updates) => { await updateCampaign(campaignId, updates); setEditModalOpen(false); }}
