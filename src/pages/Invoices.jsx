...
import {
  ...
  ChevronDown, Trash2, Edit3, ExternalLink, Copy, Filter,
} from 'lucide-react';
...
// SQL instructions for when the table is missing
...
export default function Invoices({ clients = [] }) {
  ...
  const openAmount   = invoices.filter((i) => i.status === 'sent').reduce((s, i) => s + getTotal(i), 0);
  ...
  const handleStatusChange = async (inv, newStatus) => {
    const result = await updateInvoice(inv.id, { status: newStatus });
    if (result.error) return;
    // Mark linked tasks paid if status = paid
    if (newStatus === 'paid' && inv.task_ids?.length) {
      const uids = Array.isArray(inv.task_ids) ? inv.task_ids : JSON.parse(inv.task_ids || '[]');
      if (uids.length) {
        const { supabase } = await import('../lib/supabase');
        await Promise.all(uids.map((tid) => supabase.from('tasks').update({ paid: true }).eq('id', tid).eq('user_id', user?.id)));
      }
    }
  };
  ...
  if (error === 'table_missing') {
    return (
      <div className="p-6 md:p-10 max-w-2xl">
        <h1 className="font-display text-2xl font-bold text-gray-900 mb-2">Invoices</h1>
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 space-y-4">
          ...
          <pre className="bg-white border border-amber-200 rounded-xl p-4 text-xs font-mono text-gray-700 overflow-x-auto whitespace-pre-wrap">{SETUP_SQL}</pre>
          <button onClick={fetchInvoices} className="px-4 py-2 rounded-xl text-sm font-medium text-white" style={{ backgroundColor: 'var(--accent)' }}>
  ...
  return (
    <div className="p-4 md:p-8 page-enter">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-display text-2xl font-bold text-gray-900">Invoices</h1>
        <button
      ...
      <div className="bg-white rounded-2xl shadow-sm px-4 py-3 mb-4 flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-2 ml-auto">
          <button onClick={() => setShowFilters((f) => !f)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium text-gray-500 bg-gray-100 hover:bg-gray-200 transition-colors">
        ...
        {showFilters && (
          <div className="w-full flex items-center gap-3 pt-2 border-t border-gray-100">
            <div className="flex items-center gap-2">
              ...
              <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="px-2 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-xs outline-none focus:border-gray-400" />
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-400">To</span>
              <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="px-2 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-xs outline-none focus:border-gray-400" />
            </div>
            {(dateFrom || dateTo) && (
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        {loading ? (
          <div className="py-16 text-center">
            <div className="w-8 h-8 border-2 border-gray-200 border-t-transparent rounded-full animate-spin mx-auto mb-3" style={{ borderTopColor: 'var(--accent)' }} />
          ...
          <div className="py-16 flex flex-col items-center gap-3">
            ...
            <button
              onClick={() => setShowNewModal(true)}
              className="mt-1 flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium text-white hover:opacity-90 transition-opacity"
              style={{ backgroundColor: 'var(--accent)' }}
            >
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((inv) => {
                  ...
                  const clientName = billTo.name || (clients.find((c) => c.id === inv.client_id)?.name) || '—';
                  ...
                  return (
                    <tr key={inv.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors group">
                      <td className="px-5 py-3.5">
                        <button onClick={() => navigate(`/invoices/${inv.id}`)} className="font-mono text-sm font-semibold hover:underline" style={{ color: 'var(--accent)' }}>
                          {inv.invoice_number}
                        </button>
                      </td>
                      ...
                      <td className="px-3 py-3.5">
                        <StatusDropdown inv={inv} onStatusChange={handleStatusChange} cfg={cfg} />
                      </td>
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => navigate(`/invoices/${inv.id}`)} className="p-1.5 rounded-lg hover:bg-gray-200 text-gray-400 hover:text-gray-600 transition-colors" title="Edit">
                            <Edit3 size={13} />
                          </button>
                          {inv.share_enabled && inv.share_token && (
                            <button onClick={() => handleCopyLink(inv)} className="p-1.5 rounded-lg hover:bg-gray-200 transition-colors" title="Copy link" style={{ color: copiedId === inv.id ? 'var(--accent)' : undefined }}>
                              {copiedId === inv.id ? <CheckCircle2 size={13} /> : <Copy size={13} className="text-gray-400" />}
                            </button>
                          )}
                          <button onClick={() => handleDelete(inv.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors" title="Delete">
                            <Trash2 size={13} />
                          </button>
                        </div>
                      </td>
