...
-- ── 4. F5: drop the over-permissive /share public policies ───────────────────
-- The /share feature is defunct in this deployment: ShareModal is never rendered
-- anywhere in the app, all shared_client* tables are empty, and the /share page
-- reads tables that don't exist (client_files, task_files, client_campaigns…).
-- These policies are flag-gated (true / active=true) rather than token-bound, so
-- they're the F5 leak. Owner policies remain; nothing reachable in the app uses
-- these public ones (the live entry read goes through get_shared_client).
drop policy if exists "shared_clients_select_public"        on public.shared_clients;
drop policy if exists "shared_client_invites_select_public" on public.shared_client_invites;
drop policy if exists "shared_client_members_select_public" on public.shared_client_members;
